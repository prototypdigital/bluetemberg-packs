...
3. The agent MUST gather the following — if any is unclear, ask:
   ...
   - **tools**: the minimal tool set the role needs, as a string array (e.g. `["read", "search"]` for a reviewer; add `"edit"` if it must modify files). Default to least privilege — never grant `edit`/`execute` to a read-only role. `"execute"` MUST always be paired with `"edit"` (`["read", "search", "edit", "execute"]`) — an `execute`-only grant with no `edit` does not resolve to any tools at runtime. If a role truly must run commands but never touch files (e.g. a headless reviewer that only calls `gh`), grant `edit` anyway and add an explicit constraint prohibiting file edits (see `pr-reviewer` for the pattern) rather than omitting it.
